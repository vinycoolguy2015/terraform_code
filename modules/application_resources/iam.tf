...
resource "aws_iam_role" "ecs_cluster_autoscalingrole" {
  name               = "${local.ecs_cluster_name}-AutoscalingRole"
...
resource "aws_iam_role_policy" "ecs_cluster_autoscalingpolicy" {
  name   = "${local.ecs_cluster_name}-AutoscalingPolicy"
...
resource "aws_iam_role" "ecs_cluster_ecsrole" {
  name               = "${local.ecs_cluster_name}-ECSRole"
  assume_role_policy = <<EOF
{ 
"Version": "2012-10-17",
"Statement": [
...
resource "aws_iam_role_policy" "ecs_cluster_policy" {
  name   = "${local.ecs_cluster_name}-ECSClusterPolicy"
...
resource "aws_iam_role" "ecs_cluster_ecstaskrole" {
  name               = "${local.ecs_cluster_name}-ECSTaskRole"
  assume_role_policy = <<EOF
{ 
"Version": "2012-10-17",
"Statement": [
...
resource "aws_iam_role_policy" "ecs_cluster_ecstaskpolicy" {
  name   = "${local.ecs_cluster_name}-ECSTaskPolicy"
